...
class MaintenanceCostVisitor implements Visitor {
  visitCar(car: Car): void {
    ...
    const cost = car.getKilometers() * 0.1 + (currentYear - car.getYear()) * 50;
  ...
  visitMotorcycle(motorcycle: Motorcycle): void {
    const currentYear = (new Date()).getFullYear();
    const cost = motorcycle.getKilometers() * 0.05 + (currentYear - motorcycle.getYear()) * 30;
  ...
  visitTruck(truck: Truck): void {
    const currentYear = (new Date()).getFullYear();
    const cost = truck.getKilometers() * 0.15 + truck.getLoadCapacity() * 20 + (currentYear - truck.getYear()) * 100;
